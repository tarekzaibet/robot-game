status_board = {
	...
	dialogue = {
		...
		maximum_number_of_lines = 20;	-- Max number of displayed lines (vertical arrangement)
...
function status_board.UpdateContent()
	...
	-- For now, we don't have much to print
		...
		--status_board.message.content = BOARD_MESSAGE_DISTANCE..tostring(Gus.memory.environement[step_taken])..BOARD_MESSAGE_KM
		if tag == "step_taken" then
		status_board.message.content = BOARD_MESSAGE_KM..tostring(Gus.memory.environement[tag])
		Gus.steps=Gus.memory.environement[tag]
		--print ("steps")
		--print(Gus.steps)
		else 
		status_board.message.content = BOARD_MESSAGE_WALL..tostring(Gus.memory.environement[tag])
		Gus.wall_hits =Gus.memory.environement[tag]
		--print("hits")
		--print(Gus.wall_hits)
		end
	else
...
function status_board.dialogue.trimAndAdd(addition)
	...
	if table.getn(status_board.dialogue.content) > status_board.dialogue.maximum_number_of_lines then
		table.remove(status_board.dialogue.content, 1)
		table.remove(status_board.dialogue.content, 2)
		
		
		
	
	
		-- This content is lost
	end
end
