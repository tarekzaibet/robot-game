...
function NPC:isWillingToProvidePersonalData()
	...
	if  self.profile == NPC_PROFILE_AGGRESSIVE or self.profile == NPC_PROFILE_CHARMER or self.profile == NPC_PROFILE_ARROGANT then--or self.profile == NPC_PROFILE_CHARMER or NPC_PROFILE_ARROGANT then
		return false
	else
...
-- Fonction qui renvoie vrai si le PNJ est sensible à la corruptuon
-- Output : bool
function NPC:canBeCorrupted()
		if self.profile == NPC_PROFILE_SHY or self.profile == NPC_PROFILE_CHARMER  then 
			return false 
		else 
			return true
		end 
