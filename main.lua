...
-- (if you still use a main(), it'll be called before love.load()).
	...
	gauge_patience		= Gauge:new{
		...
		container_x = love.graphics.getWidth()-GAUGE_CONTAINER_WIDTH-110;
		container_y = love.graphics.getHeight()-GAUGE_CONTAINER_HEIGHT-30;
		caption_x = love.graphics.getWidth()-150;
		caption_y = love.graphics.getHeight()-35;
		label = GUS_PATIENCE_LABEL;
	}
	
	
	--Création de la gauge argent 
	gauge_argent		= Gauge:new{
		container_imgfile = "assets/img/Gauge_Container.png";
		caption_imgfile = "assets/img/Gauge_Money_Caption.png";
		container_x = love.graphics.getWidth()-GAUGE_CONTAINER_WIDTH-150;
		container_y = love.graphics.getHeight()-GAUGE_CONTAINER_HEIGHT-30;
		caption_x = love.graphics.getWidth()-190;
		caption_y = love.graphics.getHeight()-35;
		label = GUS_MONEY_LABEL;
	}
	
	
	-- NPC instances
	NPC1 = createNPC("Madder")
	NPC2 = createNPC("Britney")
	NPC3 = createNPC("Tarek")
	NPC4 = createNPC("Jean")
	...
	gauge_argent.container_img = love.graphics.newImage(gauge_argent.container_imgfile) -- Gauge: argent (container)
	gauge_argent.caption_img = love.graphics.newImage(gauge_argent.caption_imgfile)-- Gauge: argent (caption)
	-- NPCs
	...
	--Set quest 
	--quest_meet_everyone:assignQuest(NPC4)
	
	-- attribue les quetes aux différents PNJ
	quest_do_sports:assignQuest(NPC4)
	quest_find_him:assignQuest(NPC1)
	quest_get_hurt:assignQuest(NPC2)
